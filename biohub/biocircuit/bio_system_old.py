...
try:
    GATE_FILE = open(real_path + "gates_lizhi.json", "r")
except IOError:
    GATE_FILE = open(real_path + "gates_lizhi.json", "r")
...
def dev_system(gates, data, nodes_id, input_sub, output_sub):
    ...
    tmp = [replace_str(species, "S" + nodes_id + gates["output"][i], "S" + output_sub[i]) for i in
           range(len(output_sub))]
    tmp = [replace_str(reaction, "S" + nodes_id + gates["input"][i], "S" + input_sub[i]) for i in range(len(input_sub))]
    tmp = [replace_str(reaction, "S" + nodes_id + gates["output"][i], "S" + output_sub[i]) for i in
           range(len(output_sub))]
    """
